  ...
  settingsType = types.submodule {
    ...
    options = {
      log-level = lib.mkOption {
        type =
          types.enum [ "trace" "debug" "info" "warn" "error" "fatal" "panic" ];
      ...
      log-output = lib.mkOption {
        ...
        description = lib.mdDoc
          "Log output location; comma separated list of `stdout` and/or `file`";
        default = "stdout";
      };
      ...
      opengist-home = lib.mkOption {
        type = types.path;
        description =
          lib.mdDoc "Path to the directory where opengist stores its data";
        readOnly = true;
        default = stateDir;
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.opengist = {
      ...
      serviceConfig = {
        ...
        ExecStart =
          "${lib.getBin cfg.package}/bin/opengist -config ${cfg.settingsFile}";
        ...
        SystemCallFilter =
          [ "@system-service" "~@privileged @obsolete" "@pkey" ];
        PrivateDevices = true;
        PrivateUsers = true;
