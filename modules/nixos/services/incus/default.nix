...
let
  ...
  imageAlias = instance: if isLocal instance then instance.name else instance.image;

  applyDir = pkgs.linkFarm "incus-apply-config" (
    map (instance: {
  ...
  importScript = lib.concatMapStrings (
    instance:
    lib.optionalString (isLocal instance) ''
      if ! incus image info ${lib.escapeShellArg instance.name} &>/dev/null; then
        incus image import \
...
in
{
        ...
        types.submodule {
          options = {
            ...
            image = mkOption {
              ...
              description = "Image to use (either a local QEMU image or a reference to a remote image)";
            };

            metadata = mkOption {
              type = types.nullOr types.package;
              default = null;
              description = "Image metadata derivation (required when image is a local derivation).";
            };

            profiles = mkOption {
              type = types.listOf types.str;
              description = "Which Incus profiles should be used for this instance";
              default = [ "default" ];
            };

            vm = mkOption {
              type = types.bool;
              description = "If this instance should be a VM instead of a container";
              default = false;
            };
  ...
  config = mkIf cfg.enable {
    assertions = map (instance: {
      ...
      message = ''virtualisation.incus.instances: instance "${instance.name}" has a local image derivation but no metadata set.'';
    }) cfg.instances;
            ...
            text =
              let
                configLines = lib.concatStringsSep "\n" (
                  lib.mapAttrsToList (k: v: "incus config set ${lib.escapeShellArg "${k}=${v}"}") (
                    flatten "" cfg.config
                  )
                );
              in
