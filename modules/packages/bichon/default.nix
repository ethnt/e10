{ stdenv, lib, rustPlatform, fetchFromGitHub, fetchPnpmDeps, nodejs_22, pnpm
, pnpmConfigHook, typescript, pkg-config, git, openssl }:
...
rustPlatform.buildRustPackage (let
  ...
  frontend = stdenv.mkDerivation (finalAttrs: {
    ...
    nativeBuildInputs = [ nodejs_22 pnpm pnpmConfigHook typescript ];
...
in {
  ...
  meta = with lib; {
    description =
      "Lightweight, high-performance Rust email archiver with WebUI";
