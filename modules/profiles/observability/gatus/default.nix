{ config, lib, flake, hosts, ... }: {
  ...
  services.gatus = {
    ...
    settings = {
      ...
      endpoints = let
        providedEndpoints = lib.mapAttrsToList (_name: service:
          {
            inherit (service.monitor) name group conditions interval url;
            enabled = true; # TODO
            alerts = [{
              enabled = true;
              type = "ntfy";
          ...
          } // service.monitor.extraConfig
          // lib.optionalAttrs service.http.proxy.protected {
            headers = { Authorization = "Basic $AUTHELIA_BASIC_AUTH"; };
          }) (lib.filterAttrs (_name: service: service.monitor.enable)
            (flake.lib.provides.allServicesForHosts hosts));
        # bastion = [
        #   (mkEndpoint {
        #     name = "Caddy";
        ...
        # ++ monitor ++ pikvm ++ providedEndpoints;
      in providedEndpoints;
    };
  };
