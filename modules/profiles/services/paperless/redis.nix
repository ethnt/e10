{ config, ... }: {
  ...
  services.paperless.settings.PAPERLESS_REDIS = "unix://${config.services.redis.servers.paperless.unixSocket}";

  systemd.services =
    let
      additionalServiceConfig = {
        serviceConfig.SupplementaryGroups = config.services.redis.servers.paperless.user;
