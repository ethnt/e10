  ...
  services.grafana = {
    settings = {
      ...
      "auth.generic_oauth" = {
        ...
        client_secret = "$__file{${config.sops.secrets.grafana_authelia_client_secret.path}}";
      ...
      smtp = {
        ...
        password = "$__file{${config.sops.secrets.grafana_smtp2go_password.path}}";
        ...
        datasources = [
          {
            ...
            type = "prometheus";
            access = "proxy";
            url = "http://${hosts.monitor.config.networking.hostName}:${toString config.services.thanos.query.http.port}";
          }
          {
            name = "Loki";
            type = "loki";
            access = "proxy";
            url = "http://${hosts.monitor.config.networking.hostName}:${toString config.services.loki.configuration.server.http_listen_port}";
          }
          {
            name = "PostgreSQL";
            type = "postgres";
              ...
              receivers = [
                {
                  ...
                  type = "email";
                  settings = {
                    addresses = "ethan+e10@turkeltaub.dev";
                  };
                }
              ];
            }
